...
public class JdbcType2JavaType {
	...
	private static Map<Integer, Class<?>> jdbcJavaTypes; // jdbc_type to
															// java_type
	static {
		...
		// 初始化jdbcJavaTypes：
		jdbcJavaTypes.put(new Integer(Types.LONGNVARCHAR), String.class); // -16字符串
		...
		jdbcJavaTypes.put(new Integer(Types.LONGVARBINARY), Blob.class); // -4二进制
		...
		jdbcJavaTypes.put(new Integer(Types.LONGVARCHAR), String.class); // -1字符串
	...
	public static int getJdbcCode(String jdbcName) {
		Integer integer = jdbcTypes.get(jdbcName);
		return null == integer ? Types.VARCHAR : integer;
	}
	...
	public static Class<?> jdbcTypeToJavaType(int jdbcType) {
		Class<?> clazz = jdbcJavaTypes.get(jdbcType);
		return null == clazz ? String.class : clazz;
	}

	public static String getJavaTypeFromJdbcType(String jdbcType) {
		return jdbcTypeToJavaType(getJdbcCode(jdbcType)).getSimpleName();
	}
